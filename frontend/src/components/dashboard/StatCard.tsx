    ...
    return (
      <div className="rounded-3xl bg-white p-6 shadow-sm ring-1 ring-red-200">
        <div className="flex items-start justify-between gap-3">
          <div>
            <p className="text-xs font-semibold tracking-[0.2em] text-slate-700">{item.title}</p>
  ...
  return (
    <div className="rounded-3xl bg-white p-6 shadow-sm">
      ...
      <div className="mt-7">
        ...
        <p className={`mt-2 text-sm font-medium ${item.positive ? 'text-emerald-600' : 'text-red-500'}`}>
          {item.change}
        </p>
      </div>
    </div>
