...
export default function CreateInvoicePage() {
  ...
  const totals = useMemo(() => {
    ...
    const discount = items.reduce((sum, item) => sum + item.total_discount_amount, 0)
    const tax = 0
    const billed = subtotal - discount + tax
    return { subtotal, discount, tax, billed }
  }, [items])

  useEffect(() => {
    setTotalPayable((current) => {
      if (current === null || Number.isNaN(current)) {
        return totals.billed
      }

      return Math.min(Math.max(current, 0), totals.billed)
    })
  }, [totals.billed])

  useEffect(() => {
    setPaidAmount((current) => {
      const payableAmount = totalPayable ?? totals.billed
      return Math.min(Math.max(current, 0), payableAmount)
    })
  }, [totalPayable, totals.billed])
  ...
  const buildPayload = (): InvoiceCreatePayload => ({
    ...
    payment_mode: paymentMode || null,
    paid_amount: paidAmount,
    total_payable_amount: totalPayable ?? totals.billed,
    total_tax_amount: totals.tax,
    invoice_status: 'saved',
    notes,
  })
  ...
  const loadInvoiceForEdit = async () => {
    ...
    try {
      ...
      setItems((data.items || []).map(mapInvoiceItemToLocalItem))
      setPaidAmount(Number(data.paid_amount || 0))
      setTotalPayable(Number(data.final_amount || 0))
      setPaymentMode((data.payment_mode as PaymentMode | null) || '')
      setPaymentStatus((data.payment_status as PaymentStatus) || 'pending')
      setNotes(data.notes || '')
      setInvoiceDate(data.invoice_date || new Date().toISOString().slice(0, 10))
        ...
        >
          ...
          <div className="grid gap-8 xl:grid-cols-[1fr_410px]">
            ...
            <InvoiceSummaryCard
              ...
              totalPayable={totalPayable ?? totals.billed}
